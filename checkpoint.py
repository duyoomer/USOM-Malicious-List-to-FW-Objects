...
# Checkpoint config dosyası adı
output_filename = "checkpoint_objects.txt"
...
# Eğer checkpoint_objects.txt dosyası mevcutsa, sil
if os.path.exists(output_filename):
    os.remove(output_filename)
    print(f"{output_filename} dosyası silindi.")

# Checkpoint firewall için konfigürasyon dosyası oluştur
with open(output_filename, "w") as f:
    # URL listesindeki her bir benzersiz URL için Checkpoint host object oluştur
    for i, url in enumerate(unique_urls):
        if url.strip():  # Boş satırları atla
            ...
            # Checkpoint nesnesi ekleme (FQDN veya IP türüne göre)
            if object_type == "fqdn":
                f.write(f"add host name \"{object_name}\" ip-address \"{url.strip()}\" type \"fqdn\"\n")
            elif object_type == "ip":
                f.write(f"add host name \"{object_name}\" ip-address \"{url.strip()}\"\n")
...
# Sonuçları yazdır
print(f"Checkpoint firewall konfigürasyon dosyası '{output_filename}' oluşturuldu.")
